...
- 当前判断：`技能` / 名称：`【珍珑】`
- 标记：`inherited_kind, inherited_named_ability, missing_indent_for_inherited`
- 原文：【珍珑】：生命到0后，无攻击能力，【残】，战斗变为己放一张，敌猜其为五行一种；猜中则敌被逆运北冥；猜错则敌与自身相同幻象作战至胜出（杀死幻象或至打得好）（可出战次数为5+生命至0前己出场次数，有过珍珑者，自身死亡）
...
- 当前判断：`武功` / 名称：`金木水火土`
- 标记：`inherited_kind, inherited_named_ability, missing_indent_for_inherited`
- 原文：金木水火土：不会受到场景负面效果，免疫普攻

### 萧秋水 / 战斗人物!393:3

- 当前判断：`武功` / 名称：`日月风云我`
- 标记：`inherited_kind, inherited_named_ability, missing_indent_for_inherited`
- 原文：日月风云我：有一次使本回合/次全部攻击、效果透至发出者外另一处的机会
...
- 当前判断：`字` / 名称：`【树叶的叶，开心的开】`
- 标记：`implicit_word, exclusive_word`
- 原文：【树叶的叶，开心的开】：每上场一次多一次小李飞刀，起始一次；小李飞刀不能替【恶】抵挡致命或打断针对【恶】的特技；被叶开救过的人顺位不会高于叶开
...
- 当前判断：`说明` / 名称：`—`
- 标记：`author_corrected, free_text, nested_continuation_line, unit_specific_ability`
- 原文：本方遗失一件物品，一个附加人物，一人物的一张基础卡牌，一个人物的一个特技失灵（均视为针对玩家的效果） 玩家可以在石中玉不出战的一轮中选择己方某人物杀死石中玉或进行判定：1/2 石中玉逃走到另一方（石中玉不会重复逃回已经待过的一方），1/2无效 杀死石中玉则石清闵柔与其死战，石中玉被迫出战时则可以请石清闵柔出战一回合（其余时候石清闵柔不出现在场上）
