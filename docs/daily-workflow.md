...
已精评人物的完整评审、强度、定位、电子化风险和攻略写入：

- `data/review/cards/<卡名>/README.md`：总分析与维度导航；
- `data/review/cards/<卡名>/mechanics.md`等维度文件：完整证据；
- `data/review/card_evaluations.json`：供网页显示和搜索的结构化摘要；
- `data/review/card_notes/<卡名>.md`：兼容导航，不再复制完整长文。

目录和维度路径同步到`data/review/cards/index.json`。单卡理解进入`data/review/card_understanding_notes.json`。`data/card_reviews.json`仅作历史兼容，不再是新评审默认入口。这些内容不改牌面，不进入源数据库。
...
3. 讨论中可使用临时单卡工作卡；完成时必须拆入`data/review/cards/<卡名>/`，各栏目逐项独立保存，`README.md`只承担摘要和路由。
...
9. 完整证据目录必须无一遗漏地包含**正面输出**、**侧面输出**、**正面生存**、**侧面生存**、**全局影响力**、**名次保障／败局规避**、**优点**、**缺点**、**规则风险**、**电子化风险**和**作者校准**。网页`full_text`和`README.md`可以是摘要，但必须提供通往全部维度文件的明确路由。
10. 不得因当前讨论集中在输出或某一特技，就把尚未讨论的维度留到作者提醒后才补。每轮语义校准后，AI必须主动逐项检查全部证据模块；能分析的写完整推理，暂不能分析的写“待定”、缺失数据、当前假设和会改变结论的待校准问题。
11. 在向作者报告“本轮完成”或建议进入下一张前，必须先列出仍开放的规则问题，并确认`README.md`、`cards/index.json`、`comparison/`六个极简横向表、`calibrated_stats.json`、`combat_baselines.json`及进度记录是否已同步。两份详细统计逐人写入但不默认读取。存在会改变核心结论的问题时，主动向作者提问，不得用缺省沉默代替。
12. `data/review/comparison/mechanism-frequency.md`记录异常及解、出血、破、一定造成、转、连绵、清除、杀死等环境机制的全库出现频率。分析某人物的免疫、抗性、穿透或环境价值时按需读取；它是词面候选频率，不等于实战触发率，未经语义复核不得直接当概率使用。
13. `data/review/comparison/ability-type-frequency.md`记录战斗人物的特技类型、大招数量分布和统一简化放出模型。分析打断、禁大招、复制大招或“敌未使用大招”时按需读取；牌面数量、简化放出率与真实使用率必须分层。
